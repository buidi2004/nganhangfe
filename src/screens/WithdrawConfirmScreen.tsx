  ...
  React.useEffect(() => {
    const fetchFee = async () => {
      try {
        ...
        const res = await WalletApi.estimateFees(user.walletId, rawNumAmount, 'WITHDRAWAL', 'VND');
    ...
    if (pinDigits.length < 6 && !isTransferring) {
      ...
      if (nextPins.length === 6) {
        ...
        try {
          ...
          // 1. Xác thực mã PIN để lấy pinToken
          const pinRes = await WalletApi.verifyPin(nextPins.join(''));
          const pinToken = typeof pinRes.data === 'string' ? pinRes.data : (pinRes.data as any)?.pinToken;
          if (!pinToken) throw new Error('Không lấy được mã xác thực PIN');

          // 2. Thực hiện lệnh rút tiền
          const mockBankAccountId = "mock-bank-123";
          const withdrawRes = await WalletApi.withdraw(user.walletId, rawNumAmount, 'VND', mockBankAccountId, pinToken);
          ...
          navigation.navigate('TransferResult', {
            ...
            transactionId: withdrawRes.data?.id,
            timestamp: new Date().toISOString(),
          });
        } catch (e: any) {
          setIsTransferring(false);
          setPinDigits([]);
          Alert.alert('Lỗi rút tiền', e.message || 'Giao dịch thất bại');
        }
      }
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.header}>
        ...
        <AppText style={styles.headerTitle}>Xác nhận rút tiền</AppText>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.detailsCard}>
          ...
          <View style={styles.partySection}>
            <AppText style={styles.partyHeaderLabel}>Nguồn rút tiền</AppText>
          ...
          <View style={styles.partySection}>
            <AppText style={styles.partyHeaderLabel}>Tài khoản nhận</AppText>
            <View style={styles.partyInfoRow}>
              <View style={styles.vcbLogoCircle}>
                <Ionicons name="card" size={17} color="#15803D" />
              </View>
              <View style={styles.partyDetailsCol}>
                <AppText style={styles.partyName}>{selectedBank}</AppText>
        ...
        <View style={styles.otpModalOverlay}>
          ...
          <View style={styles.otpModalContent}>
            ...
            <AppText style={styles.otpModalSubtitle}>
              Mã PIN của bạn dùng để xác thực giao dịch rút tiền.
            </AppText>
            ...
            {isTransferring && (
              <View style={{ alignItems: 'center', marginTop: 15 }}>
                <ActivityIndicator size="small" color="#D2519D" />
                <AppText style={{ color: '#D2519D', marginTop: 5 }}>Đang xử lý rút tiền...</AppText>
              </View>
            )}
